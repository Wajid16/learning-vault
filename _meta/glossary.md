...
**Persistence / Checkpointer (LangGraph)** — saving a graph's state (via a "checkpointer" object) so it survives past a single `invoke()` call and can be reloaded on the next one, keyed by a "thread" (one ongoing conversation/session). Without it, every `invoke()` starts from a blank state — which looks like the app "forgetting" everything between turns. First seen in: [langgraph/notes/09-chatbot-memory-bug.md](../langgraph/notes/09-chatbot-memory-bug.md).
